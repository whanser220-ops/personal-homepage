...
流水线只保留最近 20 次构建；同一时间有新 `main` 提交到达时，会终止已过时的部署并只继续最新版本。构建和部署阶段会在临时 agent 通道中断时重新申请 agent 后重试。构建机只保留应用的 `latest` 本地标签，云主机默认保留当前版本和最近两个可回滚版本，Harbor 仍保存正式发布标签。
